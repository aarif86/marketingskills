...
<div class="card"><h2>Hosting status (self-aware banner)</h2>
<p class="muted small">Read from Hostinger’s status page every 10 minutes. When maintenance or an incident touches the Singapore/Asia servers${config.hostinger.serverName ? html` (or <code>${config.hostinger.serverName}</code>)` : ''}, every page shows a bar. Last read: ${h.hostStatus.fetched_at ? timeAgo(new Date(h.hostStatus.fetched_at).toISOString()) : 'never'}${h.hostStatus.ok === false && h.hostStatus.error ? html` · <span class="warn">could not read (${h.hostStatus.error})</span>` : ''}${h.hostStatus.overall ? html` · provider says “${h.hostStatus.overall}”` : ''}.</p>
${h.hostStatus.notices?.length ? html`<ul class="plain">${h.hostStatus.notices.map((n) => html`<li>${n.level === 'warn' ? '⚠️' : 'ℹ️'} ${n.text}</li>`)}</ul>` : html`<p class="muted small">No maintenance or incident affecting us right now.</p>`}
<form method="post" action="/admin/health/status" class="inline">${hidden(h.csrf)}<button class="btn btn-ghost btn-sm" type="submit">Check status now</button></form>
${config.hostinger.serverName ? '' : html`<p class="muted small mt">Tip: put your server name from hPanel → Hosting → Plan details into <code>HOSTINGER_SERVER</code> in nsd-data/.env so server-specific notices match too.</p>`}
</div>
